void drawInterpolation() {
  ...
  for(Tri t:tris){
    ...
    PVector pb=t.b;
    PVector pc=t.c;
    int ia=points.indexOf(pa);
    int ib=points.indexOf(pb);
    int ic=points.indexOf(pc);
    if(ia==-1||ib==-1||ic==-1)continue;
    color ca=cols.get(points.indexOf(pa));
    color cb=cols.get(points.indexOf(pb));
    color cc=cols.get(points.indexOf(pc));
    noStroke();
    beginShape(QUADS);
    fill(ca);
    vertex(pa.x,pa.y);
